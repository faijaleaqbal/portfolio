/**
 * Devini Labs Liquid Metal 3D WebGL2 Button
 * High-fidelity dispersion shader pill component from devinilabs/sylva-hero
 */
(function () {
  'use strict';
...
let gl = null;
try {
  gl = cv.getContext('webgl2', {alpha:true, antialias:false, premultipliedAlpha:true, powerPreference:'high-performance'});
} catch (e) {
  gl = null;
}
const stage = host;
...
if (!btn) return;
const plate = host.querySelector('.liquid-plate') || host.querySelector('.plate');
...
if(!gl){
  // Graceful fallback: CSS pill and plate will still look great without WebGL2
  return;
} else {
  ...
  const observer = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      if (entry.target === host) {
        ...
        if (!wasInView && inView && !frameActive) {
          frameActive = true;
...
window.__seek   = v => { clock = v; drawn = null; };
  ...
  function initLiquidButtons() {
    ...
    for (let i = 0; i < hosts.length; i++) {
      if (!hosts[i].dataset.liquidMounted) {
        hosts[i].dataset.liquidMounted = 'true';
